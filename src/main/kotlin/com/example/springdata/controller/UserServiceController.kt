...
class UserServiceController(private val userService: UserService) {
    ...
    fun createUser(@RequestBody request: CreateUserRequest): ResponseEntity<UserId> {
        val id: String = userService.createUser(request.name, request.email)
...
private fun transform(entity: UserEntity): UserData {
    return UserData(entity.id, entity.name, entity.email, entity.enabled)
}
